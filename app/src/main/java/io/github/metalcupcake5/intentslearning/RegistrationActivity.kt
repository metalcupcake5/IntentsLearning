...
class RegistrationActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        editText_registration_name.setText(username)
        ...
        button_registration_create.setOnClickListener {
            when {
                editText_registration_name.text.toString().length < 3 -> {
        ...
        Backendless.UserService.register(user, object : AsyncCallback<BackendlessUser?> {
            override fun handleResponse(registeredUser: BackendlessUser?) {
                ...
                val intent = Intent().apply {
                    putExtra(EXTRA_USERNAME, editText_registration_name.text.toString())
